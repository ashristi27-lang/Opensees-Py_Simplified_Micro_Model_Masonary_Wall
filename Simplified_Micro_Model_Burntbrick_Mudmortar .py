...
# Geometry (Standard Nepali Brick Size approx 230x110x55 or 210x105x55)
brick_L = 210.0
brick_H = 55.0
mortar_t = 10.0
wall_thick = 230.0
...
dx_mesh = dx_unit / 2.0   # Meshing the brick into 2 elements for better accuracy
dy_mesh = dy_unit

N_cols = 9
N_rows = 27
# Source: Comparable to values from Gautam et al. and Mishra et al. for Nepali Masonry
brick_E = 4300.0
brick_nu = 0.24
brick_rho = 1900.0 
...
for r in range(N_rows):
    ...
    for c in range(cols):
        ...
        # Apply load to nodes (Pattern 1 is already active)
...
for i in range(1000): 
    ...
    for idx, target in enumerate(target_snapshots):
        if not captured_flags[idx] and abs(d) >= target:
            ...
            # Save copy of current displacements
